...
for i = 1:m
    ...
    for j = 1:m
        ...
        sigma_rr(i,j) = (2*A*r(i)-2*B*r(i)^(-3)+C*r(i)^(-1)-2*D*r(i)^(-1))*sin(theta(j));
        
        % Computation of the circumferential normal stress distribution
        
        sigma_tt(i,j) = (-2*A*r(i)+2*B*r(i)^(-3)-C*r(i)^(-1))*cos(theta(j));
    
        % Computation of the tangental (shear) stress distribution
        
        tau_rt(i,j) = -(6*A*r(i)+2*B*r(i)^(-3)+C*r(i)^(-1))*sin(theta(j));
